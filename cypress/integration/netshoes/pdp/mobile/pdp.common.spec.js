    ...
    it('Validar Botão Comprar com SKU Selecionado', function () {
      ...
      cy.get('section.cfs [data-slide-push-target="selector-content-cfs"]')
        ...
        .then(($selector) => {
          ...
          // Select the first SKU on the list
          cy.get('.product__color--list').children().first().click()

          // Wait for new SKU ajax request to end
          cy.wait('@ajaxPdpShowcase')

          // Check if the OK button is hidden
          cy.get('span.close-modal-button').should('have.class', 'hide')

          // Check if the close X button is visible
          cy.get('i.ns-icon-close-x').should('not.have.class', 'hide')
            .click()
        })
      ...
      cy.get('section.cfs [data-slide-push-target="selector-content-cfs"]')
        ...
        .then(($selector) => {
          ...
          // Select second SKU
          cy.get('.product__color--list').children().eq(1)
            .click()
          ...
          // Select first Size
          cy.get('.product__size--list').children().eq(0)
            .click()
